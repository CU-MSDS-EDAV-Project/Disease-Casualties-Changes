...
To verify our previous concerns on outliers which state has the highest number of deaths because of these three death causes across the year, we would like to use a heatmap to have a more straighforward idea on the frequency of death for across the years.
```{r}
...
```{r}
data3 <- read.csv("data/data3_adult.csv")

data3$Month_Week <- sub(" .*", "", data3$Month_Week)
