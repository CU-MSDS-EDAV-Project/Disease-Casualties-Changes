...
Previously we discovered that there are similar trends across diversified diseases. However, this is only our observation and needs to be verified with further details. We fulfilled this by facetting on the scatter plot of pairwise disease. 
```{r}
...
Vaccines are crucial approaches to prevent and reduce the number of deaths for diseases. After knowing the trends for various diseases, we are interested in exploring the trends for vaccines as well either nationally or statewise.
```{r}
# Read data3_adult
